...
}-${startDate.getDate()}T06:00:00+00:00","${endDate.getFullYear()}-${
  endDate.getMonth() + 1
}-${endDate.getDate() + 1}T05:59:59+00:00",true]`;
...
axios
  .request(config)
  .then((response) => {
    fs.writeFileSync("res_800.json", JSON.stringify(response.data, null, 2));
  })
  .catch((error) => {
    console.log(error);
  });
